...
public class HEJava2Spiglet extends GJDepthFirst<Base_t, Base_t> {
    ...
    public HEJava2Spiglet(Map<String, Class_t> st, int globals, int init_heap_offset) {
        this.label_cnt_ = 0;
        this.st_ = st;
    ...
    public Base_t visit(AssignmentStatement n, Base_t argu) throws Exception {
        ...
        String expr_reg = expr.getRegister();
        Method_t meth = (Method_t) argu;
        if (meth != null) {
    ...
    public Base_t visit(IncrementAssignmentStatement n, Base_t argu) throws Exception {
        ...
        if (meth != null) {
            ...
            if (var == null) { // didn't find the var in method, so it's a field of the class
                ...
                if (var.getType().equals("EncInt")) {
                    String temp = newTemp();
                    this.asm_.append("E_CONST ").append(temp).append(" 1\n");
                    this.asm_.append("MOVE ").append(var.getRegister()).append(" E_PLUS ").append(var.getRegister()).append(" ").append(temp).append("\n");
                } else {
        ...
        if (meth != null) { // if a local var
            ...
            if (var == null) { // didn't find the var in method, so it's a field of the class
                ...
                if (var.getType().equals("EncInt")) {
                    String temp = newTemp();
                    this.asm_.append("E_CONST ").append(temp).append(" 1\n");
                    this.asm_.append("MOVE ").append(var.getRegister()).append(" E_MINUS ").append(var.getRegister()).append(" ").append(temp).append("\n");
                } else {
    ...
    public Base_t visit(BinaryExpression n, Base_t argu) throws Exception {
        ...
        switch (opcode) {
            ...
            case "E_GT": {
                String new_temp = newTemp();
                this.asm_.append("MOVE ").append(new_temp).append(" ").append(t2).append("\n");
                this.asm_.append("MOVE ").append(ret).append(" E_LT ").append(new_temp).append(" ").append(t1).append("\n");
                break;
            }
            case "E_GTE": {
                String new_temp = newTemp();
                this.asm_.append("MOVE ").append(new_temp).append(" ").append(t2).append("\n");
                this.asm_.append("MOVE ").append(ret).append(" E_LTE ").append(new_temp).append(" ").append(t1).append("\n");
                break;
            }
            default:
    ...
    public Base_t visit(IntegerLiteral n, Base_t argu) throws Exception {
        String ret = newTemp();
        if (vartype_.equals("EncInt")) {
//        if (vartype_.equals("EncInt") || vartype_.equals("EncInt[]")) {
            this.asm_.append("E_CONST ").append(ret).append(" ").append(n.f0.toString()).append("\n");
        } else {
